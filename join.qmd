...
## 데이터모델 - `datamodelr` {#sqlite3-survey-model}

`survey.db` 데이터베이스의 정보를 이용해 데이터프레임을 추출하고, 이를 `datamodelr`에 넣어 데이터베이스 모델을 시각화한다. [`datamodelr`](https://github.com/bergant/datamodelr) 패키지 `nycflights13` 사례를 참조하여 유사한 방식으로 제작한다. `datamodelr`는 [R에서 데이터 모델 다이어그램을 그리는 방법](https://github.com/bergant/datamodelr)을 제공한다. 먼저 데이터프레임만 시각화하는 작업을 진행한다.
...
2. **문제:** `LEFT JOIN`을 사용할 때 결과 집합에 포함되는 것은 무엇입니까?
   1. 오른쪽 테이블의 모든 레코드
   1. 일치하는 레코드가 없는 왼쪽 테이블의 레코드
   1. 왼쪽 테이블의 모든 레코드와 일치하는 오른쪽 테이블의 레코드
   1. 두 테이블 간 일치하지 않는 모든 레코드
